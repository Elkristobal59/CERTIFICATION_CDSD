...
1. **Model Registry Centralisé (MLflow) :** Lors de l'entraînement (`src/train.py`), le pipeline de régression Random Forest est loggué et versionné dans MLflow avec ses hyperparamètres et métriques ($R^2 = 0.73$, $\text{MAE} = 11.2\ €$).
